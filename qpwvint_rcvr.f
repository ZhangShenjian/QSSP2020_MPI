...
c
c     parallelization for receivers,
c     each process only store their own results
c     instead of the array for all to save space.
c
      use mpi
      use qpalloc
      implicit none
...
c
      ...
      integer*4 irank,lfs,ldegfs,ntotal,nfr,nrapp,nr_proc,ir_proc
...
c     ensure evenly disivible
      if(mod(nr,numprocs)==0)then
        nrapp = nr
      else
        nrapp = (floor(float(nr)/float(numprocs))+1)*numprocs
      endif
      nr_proc = int(nrapp/numprocs)
c      
      allocate(tap(0:ldegmax),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: tap not allocated!'
      allocate(ldegtap(4,ns,nr_proc),stat=ierr)
...
c
        do ir=myrank+1,nrapp,numprocs
          ...
          ir_proc = (ir-(myrank+1))/numprocs+1
          call disazi(1.d0,lats(is),lons(is),
     &                     latr(ir),lonr(ir),rn,re)
c
          dis(is,ir_proc)=dsqrt(rn**2+re**2)
c
c         determine order of differential transform
c
          if(dis(is,ir_proc).le.dis0)then
            idr(is,ir_proc)=0
          else
            idr(is,ir_proc)=min0(ndmax,idint(dlog(dis(is,ir_proc)
     &        /dis0)))
          endif
c
          if(idr(is,ir_proc).eq.0.and.ipatha.eq.0.and.
     &       dis(is,ir_proc).gt.5.d0*dabs(dpr-deps(is)))then
            idr(is,ir_proc)=min0(ndmax,1+idr(is,ir_proc))
          endif
c
          ssd(is,ir_proc)=dcmplx(dsin(dis(is,ir_proc)),0.d0)
          csd(is,ir_proc)=dcmplx(dcos(dis(is,ir_proc)),0.d0)
          ssf(is,ir_proc)=dcmplx(2.d0*
     &      dsin(0.5d0*dis(is,ir_proc))**2,0.d0)
c
...
c
          ...
          ssa(is,ir_proc)=dcmplx(dsin(azi),0.d0)
          csa(is,ir_proc)=dcmplx(dcos(azi),0.d0)
          ss2a(is,ir_proc)=dcmplx(dsin(2.d0*azi),0.d0)
          cs2a(is,ir_proc)=dcmplx(dcos(2.d0*azi),0.d0)
700       continue 
        enddo
      enddo
c
...
c
          ...
          ssb(is,ir_proc)=dcmplx(dsin(bazi),0.d0)
          csb(is,ir_proc)=dcmplx(dcos(bazi),0.d0)
        enddo
800     continue
      enddo
c
...
c
      do lf=1,nf
        do ir_proc=1,nr_proc
          ue0(lf,ir_proc)=(0.d0,0.d0)
          un0(lf,ir_proc)=(0.d0,0.d0)
          uz0(lf,ir_proc)=(0.d0,0.d0)
c
          ge0(lf,ir_proc)=(0.d0,0.d0)
          gn0(lf,ir_proc)=(0.d0,0.d0)
          gz0(lf,ir_proc)=(0.d0,0.d0)
c
          roe0(lf,ir_proc)=(0.d0,0.d0)
          ron0(lf,ir_proc)=(0.d0,0.d0)
          roz0(lf,ir_proc)=(0.d0,0.d0)
c
          uee0(lf,ir_proc)=(0.d0,0.d0)
          uen0(lf,ir_proc)=(0.d0,0.d0)
          uez0(lf,ir_proc)=(0.d0,0.d0)
          unn0(lf,ir_proc)=(0.d0,0.d0)
          unz0(lf,ir_proc)=(0.d0,0.d0)
          uzz0(lf,ir_proc)=(0.d0,0.d0)
c
          see0(lf,ir_proc)=(0.d0,0.d0)
          sen0(lf,ir_proc)=(0.d0,0.d0)
          sez0(lf,ir_proc)=(0.d0,0.d0)
          snn0(lf,ir_proc)=(0.d0,0.d0)
          snz0(lf,ir_proc)=(0.d0,0.d0)
          szz0(lf,ir_proc)=(0.d0,0.d0)
        enddo
      enddo
c
      do ig=1,ngrn
...
c
        ...
        do is=isg1(ig),isg2(ig)
          do ir_proc=1,nr_proc
            ir = 1+numprocs*(ir_proc-1)
            if(ir .le. nr)then
              nd=max0(nd,idr(is,ir_proc))
            endif
          enddo
        enddo
        write(*,*)nd,idr(0,:), myrank
c
...
c
c         broadcast GF to all processes
          ntotal = (ldegmax+1)*6
          call bcast_all_i(ldegf, 1)
          call bcast_all_c(ul0, ntotal)
          call bcast_all_c(vl0, ntotal)
          call bcast_all_c(wl0, ntotal)
          call bcast_all_c(el0, ntotal)
          call bcast_all_c(fl0, ntotal)
          call bcast_all_c(gl0, ntotal)
          call bcast_all_c(pl0, ntotal)
          call bcast_all_c(ql0, ntotal)
c
          ldegneed=0
          ldeglw=ldegmax
          do is=isg1(ig),isg2(ig)
            do ir=myrank+1,nrapp,numprocs
              if(ir.gt.nr) goto 400
              ir_proc = (ir-(myrank+1))/numprocs+1
              if(fullwave)then
                ldegtap(4,is,ir_proc)=max0(0,ldegf-idr(is,ir_proc)-1)
                ldegtap(3,is,ir_proc)=ldegtap(4,is,ir_proc)*4/5
                ldegtap(2,is,ir_proc)=0
                ldegtap(1,is,ir_proc)=0
              else
                ldegtap(4,is,ir_proc)=min0(ldegf-idr(is,ir_proc)-1,
     &                        ldeg0+idnint(rearth*PI2*f*slwupcut))
                ldegtap(3,is,ir_proc)=ldegtap(4,is,ir_proc)*4/5
                ldegtap(4,is,ir_proc)=min0(ldegf-idr(is,ir_proc)-1,
     &                                10+ldegtap(4,is,ir_proc))
                ldegtap(2,is,ir_proc)=min0(ldegtap(3,is,ir_proc),
     &                           idnint(rearth*PI2*f*slwlwcut*1.25d0))
                ldegtap(1,is,ir_proc)=ldegtap(2,is,ir_proc)*4/5
              endif
              ldegneed=max0(ldegneed,ldegtap(4,is,ir_proc))
              ldeglw=max0(0,min0(ldeglw,ldegtap(1,is,ir_proc)-
     &          idr(is,ir_proc)))
400           continue
            enddo
          enddo
...
c
     ...
     &             *cdexp(-dcmplx(-fi,f)*dcmplx(PI2*togs(is),0.d0))
            do ir=myrank+1,nrapp,numprocs
              ...
              id=idr(is,ir_proc)
c
              call taper(ldegtap(1,is,ir_proc),ldegtap(4,is,ir_proc),
     &          tap(0))
c
              call legendre(dis(is,ir_proc),plm,ldegtap(4,is,ir_proc),
     &          ldegmax)
c
              do ldeg=ldegtap(1,is,ir_proc),ldegtap(4,is,ir_proc)
c
                cfac=dcmplx(tap(ldeg),0.d0)*wavelet
                if(id.gt.0)cfac=cfac/ssf(is,ir_proc)**id
c
...
c
     ...
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *urlm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *urlm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *urlm(ldeg,6,id)*cp2*ssd(is,ir_proc)**2
c
                dut=sf3(is)*utlm(ldeg,1,id)*cp1*ssd(is,ir_proc)
     &            +expl(is)*utlm(ldeg,2,id)*cp1*ssd(is,ir_proc)
     &            +clvd(is)*utlm(ldeg,3,id)*cp1*ssd(is,ir_proc)
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *utlm(ldeg,4,id)*cp1
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *utlm(ldeg,5,id)*cp1
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *utlm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                dup=(sf1(is)*ssa(is,ir_proc)-sf2(is)*csa(is,ir_proc))
     &             *uplm(ldeg,4,id)*cp1
     &            +(ds31(is)*ssa(is,ir_proc)-ds23(is)*csa(is,ir_proc))
     &             *uplm(ldeg,5,id)*cp1
     &           +(ss12(is)*cs2a(is,ir_proc)-ss11(is)*ss2a(is,ir_proc))
     &             *uplm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                dgr=sf3(is)*grlm(ldeg,1,id)*cp0
     &            +expl(is)*grlm(ldeg,2,id)*cp0
     &            +clvd(is)*grlm(ldeg,3,id)*cp0
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *grlm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *grlm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *grlm(ldeg,6,id)*cp2*ssd(is,ir_proc)**2
c
                dgt=sf3(is)*gtlm(ldeg,1,id)*cp1*ssd(is,ir_proc)
     &            +expl(is)*gtlm(ldeg,2,id)*cp1*ssd(is,ir_proc)
     &            +clvd(is)*gtlm(ldeg,3,id)*cp1*ssd(is,ir_proc)
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *gtlm(ldeg,4,id)*cp1
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *gtlm(ldeg,5,id)*cp1
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *gtlm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                dgp=(sf1(is)*ssa(is,ir_proc)-sf2(is)*csa(is,ir_proc))
     &             *gplm(ldeg,4,id)*cp1
     &            +(ds31(is)*ssa(is,ir_proc)-ds23(is)*csa(is,ir_proc))
     &             *gplm(ldeg,5,id)*cp1
     &           +(ss12(is)*cs2a(is,ir_proc)-ss11(is)*ss2a(is,ir_proc))
     &             *gplm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                urdr=sf3(is)*errlm(ldeg,1,id)*cp0
     &            +expl(is)*errlm(ldeg,2,id)*cp0
     &            +clvd(is)*errlm(ldeg,3,id)*cp0
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *errlm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *errlm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *errlm(ldeg,6,id)*cp2*ssd(is,ir_proc)**2
c
                urdt=sf3(is)*ertlm(ldeg,1,id)*cp1*ssd(is,ir_proc)
     &            +expl(is)*ertlm(ldeg,2,id)*cp1*ssd(is,ir_proc)
     &            +clvd(is)*ertlm(ldeg,3,id)*cp1*ssd(is,ir_proc)
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *ertlm(ldeg,4,id)*cp1
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *ertlm(ldeg,5,id)*cp1
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *ertlm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                urdp=(sf1(is)*ssa(is,ir_proc)-sf2(is)*csa(is,ir_proc))
     &             *erplm(ldeg,4,id)*cp1
     &            +(ds31(is)*ssa(is,ir_proc)-ds23(is)*csa(is,ir_proc))
     &             *erplm(ldeg,5,id)*cp1
     &           +(ss12(is)*cs2a(is,ir_proc)-ss11(is)*ss2a(is,ir_proc))
     &             *erplm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                utdr=sf3(is)*etrlm(ldeg,1,id)*cp1*ssd(is,ir_proc)
     &            +expl(is)*etrlm(ldeg,2,id)*cp1*ssd(is,ir_proc)
     &            +clvd(is)*etrlm(ldeg,3,id)*cp1*ssd(is,ir_proc)
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *etrlm(ldeg,4,id)*cp1
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *etrlm(ldeg,5,id)*cp1
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *etrlm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                utdt=sf3(is)*(ett0lm(ldeg,1,id)*cp0
     &                     +ettalm(ldeg,1,id)*cp1*csd(is,ir_proc))
     &            +expl(is)*(ett0lm(ldeg,2,id)*cp0
     &                      +ettalm(ldeg,2,id)*cp1*csd(is,ir_proc))
     &            +clvd(is)*(ett0lm(ldeg,3,id)*cp0
     &                      +ettalm(ldeg,3,id)*cp1*csd(is,ir_proc))
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *(ett0lm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &           +ettalm(ldeg,4,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc)
     &            +ettblm(ldeg,4,id)*cp2*ssd(is,ir_proc))
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *(ett0lm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &           +ettalm(ldeg,5,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc)
     &              +ettblm(ldeg,5,id)*cp2*ssd(is,ir_proc))
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *(ett0lm(ldeg,6,id)*cp2*ssd(is,ir_proc)**2
     &              +ettalm(ldeg,6,id)*cp2*csd(is,ir_proc)
     &              +ettblm(ldeg,6,id)*cp2)
c
                utdp=(sf1(is)*ssa(is,ir_proc)-sf2(is)*csa(is,ir_proc))
     &             *(etp0lm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &              +etpalm(ldeg,4,id)*cp2*ssd(is,ir_proc)
     &          +etpblm(ldeg,4,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc))
     &            +(ds31(is)*ssa(is,ir_proc)-ds23(is)*csa(is,ir_proc))
     &             *(etp0lm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &              +etpalm(ldeg,5,id)*cp2*ssd(is,ir_proc)
     &          +etpblm(ldeg,5,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc))
     &          +(ss12(is)*cs2a(is,ir_proc)-ss11(is)*ss2a(is,ir_proc))
     &             *(etp0lm(ldeg,6,id)*cp2
     &              +etpalm(ldeg,6,id)*cp2*csd(is,ir_proc))
c
                updr=(sf1(is)*ssa(is,ir_proc)-sf2(is)*csa(is,ir_proc))
     &             *eprlm(ldeg,4,id)*cp1
     &            +(ds31(is)*ssa(is,ir_proc)-ds23(is)*csa(is,ir_proc))
     &             *eprlm(ldeg,5,id)*cp1
     &           +(ss12(is)*cs2a(is,ir_proc)-ss11(is)*ss2a(is,ir_proc))
     &             *eprlm(ldeg,6,id)*cp2*ssd(is,ir_proc)
c
                updt=(sf1(is)*ssa(is,ir_proc)-sf2(is)*csa(is,ir_proc))
     &             *(ept0lm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &           +eptalm(ldeg,4,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc)
     &              +eptblm(ldeg,4,id)*cp2*ssd(is,ir_proc))
     &            +(ds31(is)*ssa(is,ir_proc)-ds23(is)*csa(is,ir_proc))
     &             *(ept0lm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &           +eptalm(ldeg,5,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc)
     &              +eptblm(ldeg,5,id)*cp2*ssd(is,ir_proc))
     &           +(ss12(is)*cs2a(is,ir_proc)-ss11(is)*ss2a(is,ir_proc))
     &             *(ept0lm(ldeg,6,id)*cp2*csd(is,ir_proc)
     &              +eptalm(ldeg,6,id)*cp2)
c
                updp=sf3(is)*(epp0lm(ldeg,1,id)*cp0
     &                     +eppalm(ldeg,1,id)*cp1*csd(is,ir_proc))
     &            +expl(is)*(epp0lm(ldeg,2,id)*cp0
     &                      +eppalm(ldeg,2,id)*cp1*csd(is,ir_proc))
     &            +clvd(is)*(epp0lm(ldeg,3,id)*cp0
     &                      +eppalm(ldeg,3,id)*cp1*csd(is,ir_proc))
     &            +(sf1(is)*csa(is,ir_proc)+sf2(is)*ssa(is,ir_proc))
     &             *(epp0lm(ldeg,4,id)*cp1*ssd(is,ir_proc)
     &           +eppalm(ldeg,4,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc)
     &              +eppblm(ldeg,4,id)*cp2*ssd(is,ir_proc))
     &            +(ds31(is)*csa(is,ir_proc)+ds23(is)*ssa(is,ir_proc))
     &             *(epp0lm(ldeg,5,id)*cp1*ssd(is,ir_proc)
     &           +eppalm(ldeg,5,id)*cp2*ssd(is,ir_proc)*csd(is,ir_proc)
     &              +eppblm(ldeg,5,id)*cp2*ssd(is,ir_proc))
     &           +(ss12(is)*ss2a(is,ir_proc)+ss11(is)*cs2a(is,ir_proc))
     &             *(epp0lm(ldeg,6,id)*cp2*ssd(is,ir_proc)**2
     &              +eppalm(ldeg,6,id)*cp2*csd(is,ir_proc)
     &              +eppblm(ldeg,6,id)*cp2)
c
                ue0(lf,ir_proc)=ue0(lf,ir_proc)+dut*ssb(is,ir_proc)
     &            +dup*csb(is,ir_proc)
                un0(lf,ir_proc)=un0(lf,ir_proc)-dut*csb(is,ir_proc)
     &            +dup*ssb(is,ir_proc)
                uz0(lf,ir_proc)=uz0(lf,ir_proc)+dur
c
                ge0(lf,ir_proc)=ge0(lf,ir_proc)+dgt*ssb(is,ir_proc)
     &            +dgp*csb(is,ir_proc)
                gn0(lf,ir_proc)=gn0(lf,ir_proc)-dgt*csb(is,ir_proc)
     &            +dgp*ssb(is,ir_proc)
                gz0(lf,ir_proc)=gz0(lf,ir_proc)+dgr
c
                roe0(lf,ir_proc)=roe0(lf,ir_proc)+(0.5d0,0.d0)
     &              *( (urdp-updr)*ssb(is,ir_proc)+(utdr-urdt)
     &              *csb(is,ir_proc))
                ron0(lf,ir_proc)=ron0(lf,ir_proc)+(0.5d0,0.d0)
     &              *(-(urdp-updr)*csb(is,ir_proc)+(utdr-urdt)
     &              *ssb(is,ir_proc))
                roz0(lf,ir_proc)=roz0(lf,ir_proc)+
     &              (0.5d0,0.d0)*(updt-utdp)
c
                rot(1,1)=ssb(is,ir_proc)
                rot(1,2)=csb(is,ir_proc)
                rot(1,3)=(0.d0,0.d0)
                rot(2,1)=-csb(is,ir_proc)
                rot(2,2)=ssb(is,ir_proc)
                rot(2,3)=(0.d0,0.d0)
                rot(3,1)=(0.d0,0.d0)
                rot(3,2)=(0.d0,0.d0)
                rot(3,3)=(1.d0,0.d0)
...
c
                ...
                enddo
c
                uee0(lf,ir_proc)=uee0(lf,ir_proc)+enz(1,1)
                uen0(lf,ir_proc)=uen0(lf,ir_proc)+enz(1,2)
                uez0(lf,ir_proc)=uez0(lf,ir_proc)+enz(1,3)
                unn0(lf,ir_proc)=unn0(lf,ir_proc)+enz(2,2)
                unz0(lf,ir_proc)=unz0(lf,ir_proc)+enz(2,3)
                uzz0(lf,ir_proc)=uzz0(lf,ir_proc)+enz(3,3)
c
                eii=enz(1,1)+enz(2,2)+enz(3,3)
c
                see0(lf,ir_proc)=see0(lf,ir_proc)
     &                    +lamr*eii+(2.d0,0.d0)*muer*enz(1,1)
                sen0(lf,ir_proc)=sen0(lf,ir_proc)
     &                    +(2.d0,0.d0)*muer*enz(1,2)
                sez0(lf,ir_proc)=sez0(lf,ir_proc)
     &                    +(2.d0,0.d0)*muer*enz(1,3)
                snn0(lf,ir_proc)=snn0(lf,ir_proc)
     &                    +lamr*eii+(2.d0,0.d0)*muer*enz(2,2)
                snz0(lf,ir_proc)=snz0(lf,ir_proc)
     &                    +(2.d0,0.d0)*muer*enz(2,3)
                szz0(lf,ir_proc)=szz0(lf,ir_proc)
...
c     ig, index of GF
c
c     Send result at all processes to the master process
c     final results only at master process
c
      if(myrank .eq. 0)then
...
c
      ...
      if(myrank .eq. 0)then
c
c       receive reuslt at master process itself
c      
        do lf=1,nf
          do ir_proc=1,nr_proc
            ir = 1+numprocs*(ir_proc-1)
            ue(lf,ir)=ue0(lf,ir_proc)
            un(lf,ir)=un0(lf,ir_proc)
            uz(lf,ir)=uz0(lf,ir_proc)
c
            ge(lf,ir)=ge0(lf,ir_proc)
            gn(lf,ir)=gn0(lf,ir_proc)
            gz(lf,ir)=gz0(lf,ir_proc)
c
            roe(lf,ir)=roe0(lf,ir_proc)
            ron(lf,ir)=ron0(lf,ir_proc)
            roz(lf,ir)=roz0(lf,ir_proc)
c
            uee(lf,ir)=uee0(lf,ir_proc)
            uen(lf,ir)=uen0(lf,ir_proc)
            uez(lf,ir)=uez0(lf,ir_proc)
            unn(lf,ir)=unn0(lf,ir_proc)
            unz(lf,ir)=unz0(lf,ir_proc)
            uzz(lf,ir)=uzz0(lf,ir_proc)
c
            see(lf,ir)=see0(lf,ir_proc)
            sen(lf,ir)=sen0(lf,ir_proc)
            sez(lf,ir)=sez0(lf,ir_proc)
            snn(lf,ir)=snn0(lf,ir_proc)
            snz(lf,ir)=snz0(lf,ir_proc)
            szz(lf,ir)=szz0(lf,ir_proc)
          enddo
        enddo
c
c       receive results at slave processes
c
...
c
          ...
          enddo
c
          call MPI_RECV(ue0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(un0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(uz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
c    
          call MPI_RECV(ge0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(gn0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(gz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
c 
          call MPI_RECV(roe0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(ron0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(roz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
c
          call MPI_RECV(uee0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(uen0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(uez0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(unn0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(unz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(uzz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
c 
          call MPI_RECV(see0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(sen0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(sez0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(snn0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(snz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
          call MPI_RECV(szz0,nfr,MPI_DOUBLE_COMPLEX,irank,1,
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
c
c         store at the origianl array
          do lf = 1, nf
            do ir_proc = 1, nr_proc
              ir = (irank+1)+numprocs*(ir_proc-1)
              if(ir .gt. nr) goto 600
              ue(lf,ir)=ue0(lf,ir_proc)
              un(lf,ir)=un0(lf,ir_proc)
              uz(lf,ir)=uz0(lf,ir_proc)
c
...
c
c       send result to master process
c
        call MPI_SEND(ue0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(un0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(uz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
c
        call MPI_SEND(ge0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(gn0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(gz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
c
        call MPI_SEND(roe0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(ron0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(roz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
c
        call MPI_SEND(uee0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(uen0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(uez0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(unn0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(unz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(uzz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
c
        call MPI_SEND(see0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(sen0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(sez0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(snn0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(snz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
        call MPI_SEND(szz0,nfr,MPI_DOUBLE_COMPLEX,0,1,
     &    MPI_COMM_WORLD,ierr_mpi)
c
      endif
c
