...
c
c     parallelization for frequency samples,
...
c
      ...
      integer*4 nfapp,irank,lfs,ldegfs,ntotal,nfr,nf_proc,lf_proc
...
c
c     prepare 2D arrays at each process to store result
c
      nf_proc = int(nfapp/numprocs)
      allocate(ue0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: ue0 not allocated!'
      allocate(un0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: un0 not allocated!'
      allocate(uz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: uz0 not allocated!'
c
      allocate(ge0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: ge0 not allocated!'
      allocate(gn0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: gn0 not allocated!'
      allocate(gz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: gz0 not allocated!'
c
      allocate(roe0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: roe0 not allocated!'
      allocate(ron0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: ron0 not allocated!'
      allocate(roz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: roz0 not allocated!'
c
      allocate(uee0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: uee0 not allocated!'
      allocate(uen0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: uen0 not allocated!'
      allocate(uez0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: uez0 not allocated!'
      allocate(unn0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: unn0 not allocated!'
      allocate(unz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: unz0 not allocated!'
      allocate(uzz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: uzz0 not allocated!'
c
      allocate(see0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: see0 not allocated!'
      allocate(sen0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: sen0 not allocated!'
      allocate(sez0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: sez0 not allocated!'
      allocate(snn0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: snn0 not allocated!'
      allocate(snz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: snz0 not allocated!'
      allocate(szz0(nf_proc,nr),stat=ierr)
      if(ierr.ne.0)stop ' Error in qpwvint: szz0 not allocated!'
c     initialization
      do lf_proc=1,nf_proc
        do ir=1,nr
          ue0(lf_proc,ir)=(0.d0,0.d0)
          un0(lf_proc,ir)=(0.d0,0.d0)
          uz0(lf_proc,ir)=(0.d0,0.d0)
c
          ge0(lf_proc,ir)=(0.d0,0.d0)
          gn0(lf_proc,ir)=(0.d0,0.d0)
          gz0(lf_proc,ir)=(0.d0,0.d0)
c
          roe0(lf_proc,ir)=(0.d0,0.d0)
          ron0(lf_proc,ir)=(0.d0,0.d0)
          roz0(lf_proc,ir)=(0.d0,0.d0)
c
          uee0(lf_proc,ir)=(0.d0,0.d0)
          uen0(lf_proc,ir)=(0.d0,0.d0)
          uez0(lf_proc,ir)=(0.d0,0.d0)
          unn0(lf_proc,ir)=(0.d0,0.d0)
          unz0(lf_proc,ir)=(0.d0,0.d0)
          uzz0(lf_proc,ir)=(0.d0,0.d0)
c
          see0(lf_proc,ir)=(0.d0,0.d0)
          sen0(lf_proc,ir)=(0.d0,0.d0)
          sez0(lf_proc,ir)=(0.d0,0.d0)
          snn0(lf_proc,ir)=(0.d0,0.d0)
          snz0(lf_proc,ir)=(0.d0,0.d0)
          szz0(lf_proc,ir)=(0.d0,0.d0)
        enddo
      enddo
c
      do ig=1,ngrn
...
c       
        do lf=myrank+1,nfapp,numprocs
          ...
          lf_proc = ( lf-(myrank+1) )/numprocs + 1
          f=dble(lf-1)*df
c
          call qpqmodel(f)
          muer=cmuup(lyr)
...
c
c               results at each processor
c
                ue0(lf_proc,ir)=ue0(lf_proc,ir)+
     &              dut*ssb(is,ir)+dup*csb(is,ir)
                un0(lf_proc,ir)=un0(lf_proc,ir)-
     &              dut*csb(is,ir)+dup*ssb(is,ir)
                uz0(lf_proc,ir)=uz0(lf_proc,ir)+dur
c
                ge0(lf_proc,ir)=ge0(lf_proc,ir)+
     &              dgt*ssb(is,ir)+dgp*csb(is,ir)
                gn0(lf_proc,ir)=gn0(lf_proc,ir)-
     &              dgt*csb(is,ir)+dgp*ssb(is,ir)
                gz0(lf_proc,ir)=gz0(lf_proc,ir)+dgr
c
                roe0(lf_proc,ir)=roe0(lf_proc,ir)+(0.5d0,0.d0)
     &              *( (urdp-updr)*ssb(is,ir)+(utdr-urdt)*csb(is,ir))
                ron0(lf_proc,ir)=ron0(lf_proc,ir)+(0.5d0,0.d0)
     &              *(-(urdp-updr)*csb(is,ir)+(utdr-urdt)*ssb(is,ir))
                roz0(lf_proc,ir)=roz0(lf_proc,ir)+
...
c
                ...
                enddo
c
                uee0(lf_proc,ir)=uee0(lf_proc,ir)+enz(1,1)
                uen0(lf_proc,ir)=uen0(lf_proc,ir)+enz(1,2)
                uez0(lf_proc,ir)=uez0(lf_proc,ir)+enz(1,3)
                unn0(lf_proc,ir)=unn0(lf_proc,ir)+enz(2,2)
                unz0(lf_proc,ir)=unz0(lf_proc,ir)+enz(2,3)
                uzz0(lf_proc,ir)=uzz0(lf_proc,ir)+enz(3,3)
c
                eii=enz(1,1)+enz(2,2)+enz(3,3)
c
                see0(lf_proc,ir)=see0(lf_proc,ir)
     &                    +lamr*eii+(2.d0,0.d0)*muer*enz(1,1)
                sen0(lf_proc,ir)=sen0(lf_proc,ir)
     &                    +(2.d0,0.d0)*muer*enz(1,2)
                sez0(lf_proc,ir)=sez0(lf_proc,ir)
     &                    +(2.d0,0.d0)*muer*enz(1,3)
                snn0(lf_proc,ir)=snn0(lf_proc,ir)
     &                    +lamr*eii+(2.d0,0.d0)*muer*enz(2,2)
                snz0(lf_proc,ir)=snz0(lf_proc,ir)
     &                    +(2.d0,0.d0)*muer*enz(2,3)
                szz0(lf_proc,ir)=szz0(lf_proc,ir)
...
c
        do lf_proc = 1, nf_proc
          lf = 1+numprocs*(lf_proc-1)
          do ir = 1, nr
            ue(lf,ir)=ue0(lf_proc,ir)
            un(lf,ir)=un0(lf_proc,ir)
            uz(lf,ir)=uz0(lf_proc,ir)
c
            ge(lf,ir)=ge0(lf_proc,ir)
            gn(lf,ir)=gn0(lf_proc,ir)
            gz(lf,ir)=gz0(lf_proc,ir)
c
            roe(lf,ir)=roe0(lf_proc,ir)
            ron(lf,ir)=ron0(lf_proc,ir)
            roz(lf,ir)=roz0(lf_proc,ir)    
c
            uee(lf,ir)=uee0(lf_proc,ir)
            uen(lf,ir)=uen0(lf_proc,ir)
            uez(lf,ir)=uez0(lf_proc,ir) 
            unn(lf,ir)=unn0(lf_proc,ir)
            unz(lf,ir)=unz0(lf_proc,ir)
            uzz(lf,ir)=uzz0(lf_proc,ir)
c
            see(lf,ir)=see0(lf_proc,ir)
            sen(lf,ir)=sen0(lf_proc,ir)
            sez(lf,ir)=sez0(lf_proc,ir) 
            snn(lf,ir)=snn0(lf_proc,ir)
            snz(lf,ir)=snz0(lf_proc,ir)
            szz(lf,ir)=szz0(lf_proc,ir) 
          enddo
        enddo
c
...
c 
     ...
     &          MPI_COMM_WORLD,MPI_STATUS_IGNORE,ierr_mpi)
c
c         store at the original array
          do lf_proc = 1, nf_proc
            lf = (irank+1)+numprocs*(lf_proc-1)
            if(lf .gt. nfcut) goto 600
            do ir = 1, nr
              ue(lf,ir)=ue0(lf_proc,ir)
              un(lf,ir)=un0(lf_proc,ir)
              uz(lf,ir)=uz0(lf_proc,ir)
